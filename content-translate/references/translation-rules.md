...
SEO and format rules for translation. Loaded during the Translate step of
`content-translate`. Covers format mechanics only, formality, brand
examples, legal references, and CTA tone per locale live in
`references/cultural-adaptation.md` instead, applied in the same pass, not
a separate one.
...
7. Where it's genuinely unclear whether to keep the English term or swap
   to a local equivalent (competitive B2B terms especially), check both
   variants' real search volume in the target market via
   `seo-keyword-research` before deciding, rather than guessing.
...
- Schema JSON-LD blocks. Translate user-facing content strings such as
  ...
  Brand names, URLs, IDs, `@id`, or `sameAs`. Follow the Article/Person/
  Organization/BreadcrumbList baseline from
  `content-references/references/seo-aeo-optimization.md`'s Schema
  recommendations when adding or updating schema on the translated version.
...
| fr-BE | `,` | ` ` (NBSP) | DD/MM/YYYY | 1 234,56 EUR | « ... » |
...
- nl: `'...'` primary, straight double quotes as an accepted fallback.
...
When the source uses straight quotes, switch to the locale's primary style
above. Preserve nesting depth (outer vs. inner).
...
- [ ] No untranslated source-language fragments (except established
      loanwords like "Content Marketing" in DE or "workflow" in NL).
...
- [ ] Brand-locked terminology from the relevant `[brand]-brand-kit`
      applied, not re-derived from scratch.
